# Code review follow-up (Commit 2)

This document records the **known issues** from the original review and **what was changed** in the codebase to address them.
...
Both run `tsx src/controllers/example.ts`.

---

## Dependencies note

- Hashing uses Node’s built-in `node:crypto` (no `crypto` npm shim).
- `@types/node` supports TypeScript checking for Node APIs; `tsx` runs TypeScript without a separate build step for the demo.
